...
function process() {
    ...
    let img = new Image();
    img.onload = function() {
        ctx.drawImage(img, 0, 0, 520, 537.5);
        ctx.fillStyle = "white";
        ctx.font = "20pt Open Sans";
        ctx.fillText("Fire Emblem: 3 Houses - S ranks", 85, 45);
        if (!document.getElementById("footer").checked) {
            ctx.fillStyle = "#bbb";
            ctx.font = "8pt Open Sans";
            ctx.fillText("Generated by fireemblem.zirk.eu", 370, 535);
        }
        let i = 0;
        allCharacters.forEach(function(e) {
            if (document.getElementById(e.name).checked) {
                loadImage(ctx, "img/" + e.name + ".png", (i % 8) * 65, Math.floor(i / 8) * 97.5 + (i % 2 == 1 ? 16.25 : 0) + 8.5 + 50);
            } else {
                loadImage(ctx, "img/Empty.png", (i % 8) * 65, Math.floor(i / 8) * 97.5 + (i % 2 == 1 ? 16.25 : 0) + 8.5 + 50);
            }
            i++;
        });
    }
